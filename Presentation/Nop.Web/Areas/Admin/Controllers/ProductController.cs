...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void PrepareProductModel(ProductModel model, Product product, bool setPredefinedValues, bool excludeProperties)
        {
            ...
            /// VaskSoft 
            this.AddProductSizeAndColor(model);
        ...
        /// <summary>
        /// VaskSoft - Select Color and Size attributtes
        /// </summary>
        /// <param name="productModel"></param>
        private void AddProductSizeAndColor(ProductModel productModel)
        {
            var attributesAll = _productAttributeService.GetAllProductAttributes();
            var attributes = _productAttributeService.GetProductAttributeMappingsByProductId(productModel.Id)
               .Where(x => x.CanBeUsedAsCondition())
               .ToList();

            var attributeSize = attributes.FirstOrDefault(x => x.ProductAttribute.Name.ToLower() == "size");
            var attributeColor = attributes.FirstOrDefault(x => x.ProductAttribute.Name.ToLower() == "color");

            productModel.ProductAttributeSizeMap = attributesAll.Any(x => x.Name.ToLower() == "size");
            if (productModel.ProductAttributeSizeMap && attributeSize != null)
            {
                productModel.ProductAttributeSize = attributeSize.ProductAttributeValues.FirstOrDefault()?.Name;
            }

            productModel.ProductAttributeColorMap = attributesAll.Any(x => x.Name.ToLower() == "color");
            if (productModel.ProductAttributeColorMap && attributeColor != null)
            {
                productModel.ProductAttributeColor = attributeColor.ProductAttributeValues.FirstOrDefault()?.Name;
            }
        ...
        public virtual IActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //VaskSoft - save product color and size 
                AddProductSizeAndColor(model, product);
        ...
        /// <summary>
        /// VaskSoft - Save Color and Size attributtes
        /// </summary>
        /// <param name="productModel"></param>
        /// <param name="product"></param>
        private void AddProductSizeAndColor(ProductModel productModel, Product product)
        {
            var productAttributtes = _productAttributeService.GetAllProductAttributes();
            var colorAttributte = !string.IsNullOrEmpty(productModel.ProductAttributeColor) ? productAttributtes.FirstOrDefault(x => x.Name.ToLower() == "color") : null;
            var sizeAttributte = !string.IsNullOrEmpty(productModel.ProductAttributeSize) ? productAttributtes.FirstOrDefault(x => x.Name.ToLower() == "size") : null;
            if (colorAttributte == null && sizeAttributte == null)
            {
                return;
            }
            ...
            foreach (var item in productAttributeCombinations.ToList())
            {
                _productAttributeService.DeleteProductAttributeCombination(item);
            }
            List<int> attList = new List<int>();

            if (colorAttributte != null)
            {
                attList.Add(colorAttributte.Id);
                var productAttributeMappingNewsSelected = productAttributeMappings.FirstOrDefault(x => x.ProductAttributeId == colorAttributte.Id);
                if (productAttributeMappingNewsSelected == null)
                {
                    _productAttributeService.InsertProductAttributeMapping(new ProductAttributeMapping()
                    {
                        ProductId = product.Id,
                        ProductAttributeId = colorAttributte.Id,
                        IsRequired = true,
                        AttributeControlType = AttributeControlType.DropdownList,
                        DisplayOrder = 0
                    });

                    var productAttributeMappingNews = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id);
                    productAttributeMappingNewsSelected = productAttributeMappingNews.FirstOrDefault(x => x.ProductAttributeId == colorAttributte.Id);
                }

                if (productAttributeMappingNewsSelected != null)
                {
                    bool exist = false;
                    var atrrValues = productAttributeMappingNewsSelected.ProductAttributeValues;
                    if (atrrValues != null)
                    {
                        foreach (var atrrValue in atrrValues.ToList())
                        {
                            if (atrrValue.Name.ToLower().Trim() == productModel.ProductAttributeColor.ToLower().Trim())
                            {
                                exist = true;
                            }
                            else
                            {
                                _productAttributeService.DeleteProductAttributeValue(atrrValue);
                            }
                        }
                    }
                    if (exist == false)
                    {
                        var attributeValueColor = new ProductAttributeValue
                        {
                            ProductAttributeMappingId = productAttributeMappingNewsSelected.Id,
                            AttributeValueTypeId = 0,
                            AssociatedProductId = 0,
                            Name = productModel.ProductAttributeColor,
                            PriceAdjustment = 0,
                            WeightAdjustment = 0,
                            Cost = 0,
                        ...
                        _productAttributeService.InsertProductAttributeValue(attributeValueColor);
                    }
                }
            }

            if (sizeAttributte != null)
            {
                attList.Add(sizeAttributte.Id);
                var productAttributeMappingNewsSelected = productAttributeMappings.FirstOrDefault(x => x.ProductAttributeId == sizeAttributte.Id);
                if (productAttributeMappingNewsSelected == null)
                {
                    _productAttributeService.InsertProductAttributeMapping(new ProductAttributeMapping()
                    {
                        ProductId = product.Id,
                        ProductAttributeId = sizeAttributte.Id,
                        IsRequired = true,
                        AttributeControlType = AttributeControlType.DropdownList,
                        DisplayOrder = 0
                    });
                    ...
                    var productAttributeMappingNews = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id);
                    productAttributeMappingNewsSelected = productAttributeMappingNews.FirstOrDefault(x => x.ProductAttributeId == sizeAttributte.Id);
                }
                ...
                if (productAttributeMappingNewsSelected != null)
                {
                    ...
                    if (exist == false)
                    {
                        var attributeValueColor = new ProductAttributeValue
                        {
                            ...
                            AttributeValueTypeId = 0,
                            AssociatedProductId = 0,
                            Name = productModel.ProductAttributeSize,
                            PriceAdjustment = 0,
                            WeightAdjustment = 0,
                            Cost = 0,
                            CustomerEntersQty = false,
            ...
            if (attList.Any())
            {
                this.GenerateAllAttributeCombinations(product.Id);
            }
        }
