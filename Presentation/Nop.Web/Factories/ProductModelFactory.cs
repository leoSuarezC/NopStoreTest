...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual PictureModel PrepareProductDetailsPictureModel(Product product, bool isAssociatedProduct, out IList<PictureModel> allPictureModels)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            isAssociatedProduct = false; //A.V we show associate products same as non associated products 14-05-2019
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                var model = new ProductOverviewModel
                {
                    ...
                    StockAvailability = product.FormatStockMessage(string.Empty, _localizationService, _productAttributeParser, _dateRangeService),
                    Sku = product.Sku,
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            {
                parentProduct = _productService.GetProductById(product.ParentGroupedProductId);
            }
            ...
            var model = new ProductDetailsModel
            {
                Id = product.Id,
                Name = product.GetLocalized(x => x.Name),
                GroupedProductName = parentProduct?.GetLocalized(x => x.Name),
                ShortDescription = product.GetLocalized(x => x.ShortDescription),
                FullDescription = product.GetLocalized(x => x.FullDescription),
                ...
                SeName = product.GetSeName(),
                AdminComment = product.GetLocalized(x => x.AdminComment),
                ProductType = product.ProductType,
