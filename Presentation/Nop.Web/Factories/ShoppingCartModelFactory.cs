...
namespace Nop.Web.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        protected virtual ShoppingCartModel.ShoppingCartItemModel PrepareShoppingCartItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            ...
            if (sci.Product.ParentGroupedProductId > 0)
                parentProduct = _productService.GetProductById(sci.Product.ParentGroupedProductId); //A.V to support grouped products in cart            
            string color = ""; string size = "";
            foreach (var item in _productAttributeParser.ParseProductAttributeMappings(sci.AttributesXml))
            {
                foreach (var attributeValue in _productAttributeParser.ParseProductAttributeValues(sci.AttributesXml, item.Id))
                {
                    if (item.ProductAttribute.Name == "Color")
                    {
                        color = attributeValue.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id);
                    }
                    if (item.ProductAttribute.Name == "Size")
                    {
                        size = attributeValue.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id);
                    }
                }
            }
            //end A.V
            var cartItemModel = new ShoppingCartModel.ShoppingCartItemModel
            {
                ...
                ProductSeName = parentProduct == null ? sci.Product.GetSeName() : parentProduct.GetSeName(),
        ...
        protected virtual ShoppingCartModel.OrderReviewDataModel PrepareOrderReviewDataModel(IList<ShoppingCartItem> cart)
        {
            ...
            if (selectedPaymentMethodSystemName.IndexOf("_") > 0) //A.V MPay24 Options
            {
                selectedPaymentMethodSystemName = selectedPaymentMethodSystemName.Substring(0, selectedPaymentMethodSystemName.IndexOf("_"));
            }
            var paymentMethod = _paymentService.LoadPaymentMethodBySystemName(selectedPaymentMethodSystemName);
            string displayPaymentName = _workContext.CurrentCustomer.GetAttribute<string>(SystemCustomerAttributeNames.SessionMPay24PaymentOption); //A.V
            model.PaymentMethod = paymentMethod != null
                ? displayPaymentName //paymentMethod.GetLocalizedFriendlyName(_localizationService, _workContext.WorkingLanguage.Id)
                : "";
