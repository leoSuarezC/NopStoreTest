...
namespace Nop.Web.Controllers
{
    public partial class CommonController : BasePublicController
    {
        ...
        public virtual IActionResult ContactUs()
        {
            ...
            // When out of stock and user clicked the request item button
            var request = HttpContext.Request;
            var id = request.Query["outofstockid"].ToString();
            var farbe = request.Query["color"].ToString();
            var groesse = request.Query["size"].ToString();
            if (!string.IsNullOrEmpty(id))
            {
                var product = _productService.GetProductById(int.Parse(id));
                model.RequestProduct = new OutOfStockProduct
                {
                    Id = product.Id,
                    Name = product.Name,
                    //Url = product.GetSeName(),
                    Url = Url.RouteUrl("Product", new { SeName = product.GetSeName() }),
                    Color = farbe,
                    Size = groesse
                };
                //model.PreFilledSubject = product.Name + ", Farbe: " + farbe + ", Größe: " + groesse; // + " " + Url.RouteUrl("Product", new { product.GetSeName() });
            }
            return View(model);
        }
