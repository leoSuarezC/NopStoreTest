...
namespace Nop.Services.Catalog
{
    ...
    public partial class ManufacturerService : IManufacturerService
    {
            ...
            bool showHidden = false)
        {
            ...
            // CG: only show manufacturers that are mapped to active products
            query = (from m in query 
                join pm in _productManufacturerRepository.Table
                on new { Id = m.Id } equals new { Id = pm.ManufacturerId } into pm_join
                from pm in pm_join.DefaultIfEmpty()
                join p in _productRepository.Table on new { ProductId = pm.ProductId } equals new { ProductId = p.Id } into p_join
                from p in p_join.DefaultIfEmpty()
                where pm.Product != null && p != null && p.Deleted == false
                select m).Distinct();
